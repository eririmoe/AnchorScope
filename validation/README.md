...
`real_10k_v2` was generated from `examples2/10K_test.fastq` using `examples2/config_v2.json` with two worker processes:

```bash
anchorscope run \
  --fastq examples2/10K_test.fastq \
  --config examples2/config_v2.json \
  --outdir validation/real_10k_v2 \
  --threads 2
```

The run processed 10,000 reads and produced a machine-readable summary and MultiQC custom-content file. It is an end-to-end smoke test of real FASTQ parsing and report generation, not a biological performance benchmark or ground truth.

The HTML report is reproducible from the command above and intentionally ignored to avoid committing a large generated file. The local file remains available for manual review.

## Public 10x 5-prime Oxford Nanopore example

The manuscript example uses the first 10,000 complete FASTQ records from the public
